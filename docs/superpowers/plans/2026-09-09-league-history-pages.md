...
- **Loader output is counts only.** Exact formats: `catalog: <n> rows, <n> updated, <n> unresolved parties` and `results: <n> seasons, <n> updated, <n> unresolved names, <n> weeks with no count`, plus `skipped: <n>` on stderr-free skips. No member name, player name, nickname, or source text is ever printed.

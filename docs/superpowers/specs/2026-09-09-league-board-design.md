...
9. A doubt Sleeper has stopped projecting counts as out. A starter carrying any known flag — `Questionable`, `Doubtful`, `NA` included — with no projection at all is reported as out (tooltip `Doubtful, no projection`) rather than as missing data, because a withdrawn number behind a flag Sleeper itself raised is Sleeper saying it does not expect him on the field; a `Questionable` or `Doubtful` starter who still has a projection stays available, and a starter with no flag and no projection is still `partial`.
10. (2026-09-10) The board is one column at every breakpoint, held to `max-w-2xl` so the header and the cards share a readable measure — Ben: "make the board always one column. I find it confusing to have 2 cols on a 1-18 ranked board." — because a 1-18 ranking only reads down a single column, and `/trades` and `/history` keep their two-up grids at `sm`.

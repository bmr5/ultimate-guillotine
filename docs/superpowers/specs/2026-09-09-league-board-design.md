...
5. Injury status is synced. `public.players` gains `injury_status`, check-constrained to the nine values Sleeper's players feed emits (`Questionable`, `Doubtful`, `Out`, `IR`, `PUP`, `Sus`, `NA`, `COV`, `DNR`), written by `ug sleeper players` and carried onto every roster row the board renders. The job moves to every four hours, because the flag is the one thing on the directory that changes mid-week.
6. Out starters are reported as out, not as missing coverage. A starter whose status is `Out`, `IR`, `PUP`, `Sus`, `COV` or `DNR` leaves the coverage denominator exactly as an empty slot already did, and the card says `N starter(s) out` in the warning token. `partial` is left for a shortfall the injuries do not explain — projected starters over the slots that had a fit player in them, against the same 95 percent gate. `Questionable` and `Doubtful` tag the roster row without taking anyone out of the lineup.
7. The chips sit on the owner's line and the collapsed summary has a fixed height. Out chip first, then `partial`, each explaining itself in a `title` and in visually hidden text rather than a tooltip control — a control cannot nest inside the summary button, and the summary is one tap target. The chip row is mounted whether or not it holds anything and never wraps, the team name truncates, and a `min-h` on the summary makes every card in the grid the same height.

-- Return the top ten ranks, including boundary ties. Cards initially show five.
...
returns jsonb language sql stable security invoker set search_path = '' as $$
  ...
  select pg_catalog.jsonb_build_object(
    'highs',coalesce((select pg_catalog.jsonb_agg(
      ...
      order by points desc,season,week,key) from ranked r where high_rank <= 10),'[]'::jsonb),
    'lows',coalesce((select pg_catalog.jsonb_agg(
      ...
      order by points,season,week,key) from ranked r where low_rank <= 10),'[]'::jsonb),
    'full_lineup_lows',coalesce((select pg_catalog.jsonb_agg(
      ...
      order by points,season,week,key) from full_ranked r where full_rank <= 10),'[]'::jsonb),

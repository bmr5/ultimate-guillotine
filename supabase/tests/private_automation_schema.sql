...
-- on private.expected_runs, private.member_aliases, and private.member_contacts
-- -- installer-managed configuration, not league facts, so the worker login may
-- replace their contents -- and on public.roster_holdings, which is a
-- current-state cache of who holds whom rather than a league fact: a dropped
-- player must vanish.
select is_empty(
  $$select 1 from information_schema.role_table_grants
     where grantee = 'automation_worker'
       ...
       and not (table_schema = 'private'
                and table_name in ('expected_runs', 'member_aliases', 'member_contacts'))
  ...
  'expected_runs, member_aliases, member_contacts, and roster_holdings'
);
...
-- (d3) automation_worker does hold DELETE on private.member_contacts: the
-- handle loader replaces a member's hashed handles wholesale, so a handle that
-- left the file has to be able to leave the table.
select isnt_empty(
  $$select 1 from information_schema.role_table_grants
     where grantee = 'automation_worker'
       and table_schema = 'private'
       and table_name = 'member_contacts'
       and privilege_type = 'DELETE'$$,
  'automation_worker holds a DELETE grant on private.member_contacts'
);

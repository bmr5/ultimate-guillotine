  ...
  try {
    ...
    if (!settings.bloom) {
      return {
        tier,
        prepare: () =>
          field.compile({ colors: [output.format] }).then(() => undefined),
    ...
    const scene = own(
      target(gpu, { size: [width, height], format: HDR_FORMAT }),
    );
    const bloom = [
      own(
        target(gpu, { size: bloomSize([width, height]), format: HDR_FORMAT }),
      ),
      own(
        target(gpu, { size: bloomSize([width, height]), format: HDR_FORMAT }),
      ),
    ] as const;
    const extract = effect(gpu, bloomExtractWgsl, {
      set: { samp, src: scene },
    });
    const blurH = effect(gpu, bloomBlurWgsl, {
      set: {
        samp,
        src: bloom[0],
        blur: { texelSize: bloom[0].texelSize, direction: [1, 0] },
      },
    });
    const blurV = effect(gpu, bloomBlurWgsl, {
      set: {
        samp,
        src: bloom[1],
        blur: { texelSize: bloom[1].texelSize, direction: [0, 1] },
      },
    });
    const composite = effect(gpu, compositeWgsl, {
      set: { samp, scene, bloom: bloom[0] },
    });

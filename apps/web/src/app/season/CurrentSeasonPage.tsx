...
import { WeeklyScores } from "@/history/season/WeeklyScores";
...
export function CurrentSeasonPage() {
  ...
  const view = params.get("view") === "events" ? "events" : "scores";
  ...
  return (
    <section className="space-y-6">
      <header className="space-y-2 pt-2">
        ...
        <h2 className="text-4xl figures sm:text-5xl">Weekly results.</h2>
        <p className="max-w-2xl text-sm text-muted-foreground">
          Pick a week to see every team's score and saved roster, or revisit its
          gulag contests and cuts.
        </p>
      </header>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex gap-2" aria-label="Season views">
          {[
            ["scores", "Scores & rosters"],
            ["events", "Gulag & cuts"],
          ].map(([value, label]) => (
            <button
              type="button"
              ...
              className={cn(
                "rounded-lg border px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring",
                view === value
        ...
        <label className="flex items-center gap-3 text-sm">
          ...
          <select
            ...
            className="rounded-lg border bg-card px-3 py-2 text-foreground focus-visible:ring-2 focus-visible:ring-ring"
          >
            {season.weeks.map((w) => (
              <option key={w.number} value={w.number}>
                Week {w.number}
                {w.complete ? " · Confirmed" : ""}
              </option>
            ))}
          </select>
        </label>
        ...
        <>
            ...
            {[
              ...
              },
              {
                label: season.throughWeek
                  ...
                  : "Teams still alive",
                value:
                  season.remaining === null ? "—" : String(season.remaining),
                icon: Trophy,
              },
          ...
          <div className="space-y-4 rounded-xl border bg-card p-4 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="text-lg figures">
                18 teams. 17 weeks. One champion.
              </h3>
      ...
      {query.isPending && <LoadingState label="Loading current season" />}
      {query.isError && (
        <Alert variant="destructive">
          <AlertTitle>Could not load current season</AlertTitle>
          <AlertDescription>
              ...
              {!week.record && (
                <div className="rounded-xl border border-dashed p-8 text-center">
                  ...
                  <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                    No results recorded for Week {selected} yet. Gulag
                    qualifiers and official cuts will appear separately when the
                    week is recorded.
                  </p>
                </div>
              )}

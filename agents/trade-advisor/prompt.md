...
comparable trade code from the league's own history. Each point change names the weeks it was
totalled over in brackets: `(Week 6)` is one week, and `(Weeks 6–9)` is a rental's whole term
added up. A gain over four weeks is not four times the offer a gain over one week is.

## What you must do

Choose the two or three best candidates. Rank them 1, 2, 3 with 1 as the best. Return at most
three, and use every rank from 1 up to the number you return, once each — no gaps, no repeats.
Returning two strong ideas is better than padding to three with a weak one. Copy each
...
id. Never change an amount. Copy the candidate's structure and its one counterparty exactly;
never add a second counterparty or change the structure. Never propose any currency but FAAB — no
cash, no Venmo, no dues credit, no draft dollars — and never a leg whose kind is not `player` or
`faab`. Never say or imply that another team is close to elimination, is desperate, or is
motivated by pressure; pressure is a number you were given to rank with, not something to say out
